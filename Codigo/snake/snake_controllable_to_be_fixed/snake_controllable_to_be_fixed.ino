...
struct body* _delete(struct body* head)
{
...
void snake(int horizontallimit, int verticallimit, int sizelimit, int grouthrate, int speed, int speedup_rate)
{
	...
	*/
	...
	while(1)
	{
		...
		int gameover = _true;
                unsigned long int decision_time = millis();		
		...
		{	  
			...
			while(millis()-decision_time < 1000)
                        {
                            x = analogRead(A0);
                            y = analogRead(A1);
      
                            if(y > 900 && (x > 400 && x < 600) && attempts[0]==0) new_direction = 0;
                            if(x > 900 && (y > 400 && y < 600) && attempts[1]==0) new_direction = 1;
                            if(y < 300 && (x > 400 && x < 600) && attempts[2]==0) new_direction = 2;
                            if(x < 300 && (y > 400 && y < 600) && attempts[3]==0) new_direction = 3;
			}
			...
			{
				case 0:
					if(head->row+1 == verticalmax || (matriz[head->column][head->row+1][0] == body_color[0] && matriz[head->column][head->row+1][1] == body_color[1] && matriz[head->column][head->row+1][2] == body_color[2]))
					{
				...
				case 2:
					if(head->row == 0 || (matriz[head->column][head->row-1][0] == body_color[0] && matriz[head->column][head->row-1][1] == body_color[1] && matriz[head->column][head->row-1][2] == body_color[2]))
					{
						endgame(matriz, background_color);
				                return;
                                        }
